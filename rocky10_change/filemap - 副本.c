...
find_file_area:
	if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping)){
		//smp_rmb();
		//if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping))
			return filemap_get_folios_contig_for_file_area(mapping,index,nr_pages,pages);
	}
#endif	
	rcu_read_lock();
...
find_file_area:
	if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping)){
		//smp_rmb();
		//if(IS_SUPPORT_FILE_AREA_READ_WRITE(mapping))
			return filemap_get_folios_tag_for_file_area(mapping,index,end,tag,nr_pages,pages);
	}
#endif
			     ...
			     pgoff_t start_pgoff, pgoff_t end_pgoff)
{
	...
	folio = next_map_page_for_file_area(&xas, mapping, end_pgoff,&page_offset_in_file_area,&p_file_area);
	if (!folio)
		goto out;
	...
	do {
		...
		folio_unlock(folio);
		folio_put(folio);
	} while ((folio = next_map_page_for_file_area(&xas, mapping, end_pgoff,&page_offset_in_file_area,&p_file_area)) != NULL);
