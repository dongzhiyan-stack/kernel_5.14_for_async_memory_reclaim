		...
		const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if(file_path[count - 1] == '\n'){
		printk("echo -n /usr/bin/bash > /proc/async_memory_reclaime/file_stat_debug_or_make_backlist,or ...blacklist/usr/bin/bash...\n");
		ret = -EINVAL;
		goto free;
	}
